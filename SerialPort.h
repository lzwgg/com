...
private:  
    ...
    /** 线程退出标志变量 */   
	/* write the file again, to store the last data
		or close the file */
    static bool s_bExit;  
...
public:
		/* if the fildes is null, only display the data
		if not, then save the data to file */
	FILE *fildes;

	/* set 1 to use the file */
	UINT useFileFlag;

	/* to record the time start or time up */
	DWORD timeCounter;
	/* record the difference time */
	DWORD timeDiff;
	/* record the sum data length for one frame less than 4 */
	UINT counter;
	/* record the current 4-byte data */
	char data[8];
	/* mark the data if transfers */
	UINT dataTransferFlag;
	/* need wait 50 ms flag */
